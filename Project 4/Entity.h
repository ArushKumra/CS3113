...
enum AIState    { WALKING, IDLE, PATROLLING, JUMPING, ATTACKING };


enum AnimationDirection { STAND, JUMP, RIGHT, LEFT};
...
private:
    ...
    int m_walking[4][12]; // 4x4 array for walking animations
...
public:
    ...
    // ————— METHODS ————— //
    Entity();
    Entity(GLuint texture_id, float speed, glm::vec3 acceleration, float jump_power, int walking[4][12], float animation_time,
    ...
    void face_up() { m_animation_indices = m_walking[JUMP]; }
    void face_down() { m_animation_indices = m_walking[STAND]; }
    ...
    bool      const get_enemy_kill() const { return enemy_kill; }
    bool      const get_killed() const { return killed; }
    ...
    void const reset_kill() { enemy_kill = false; }

    // Setter for m_walking
    void set_walking(int walking[4][12])
    {
        for (int i = 0; i < 4; ++i)
        {
            for (int j = 0; j < 12; ++j)
